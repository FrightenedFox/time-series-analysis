...
```
```{r}
users.2weeks.BC.Diff.Arima42.1.21 <- Arima(users.2weeks.s24.f7,order = c(42,1,24))
```
```{r}
users.2weeks.BC.Diff.Arima42.1.0 <- Arima(users.2weeks.s24.f7,order = c(42,1,4))
```
```{r}
Acf(users.2weeks.s24.f7, lag.max = 50)
```
```{r}
autoplot(forecast(users.2weeks.BC.Diff.Arima42.1.21, h=100))
autoplot(forecast(users.2weeks.BC.Diff.Arima42.1.0, h=100))
```
```{r}
...
```
```{r}
tsdisplay(python, lag.max = 100)
tsdisplay(matlab, lag.max = 100)
```
```{r}
python.Diff <- diff(python, lag = 1)
matlab.Diff <- diff(matlab, lag = 1)
```
```{r}
tsdisplay(python.Diff, lag.max = 100)
tsdisplay(matlab.Diff, lag.max = 100)
```
